...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %                   * (aptil_p * pi_p * log(pi(+1) / pitarget) + (1 / 4) * adytil_p * muzstar_p * log(xgdp_q_obs));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  pcer_q_obs = c / c(-1) * muzstar / muzstar_p; // log(c) -log(c(-1)) + log(muzstar) -log(muzstar_p); //  consumption_obs = c / c(-1) * muzstar / muzstar_p;
  @#if cee == 0
  credit_q_obs = (q * kbar - n) / (q(-1) * kbar(-1) - n(-1)) * muzstar / muzstar_p; // credit_obs      = (q * kbar - n) / (q(-1) * kbar(-1) - n(-1)) * muzstar / muzstar_p;
  @#endif
  xgdp_q_obs = = (c + i / muup + g) / (c(-1) + i(-1) / muup(-1) + g(-1)) * muzstar / muzstar_p; //  gdp_obs         = (c + i / muup + g) / (c(-1) + i(-1) / muup(-1) + g(-1)) * muzstar / muzstar_p;
  hours_obs = h / h_ss; // hours_obs       = h / h_ss;
  pgdp_q_obs  = pi/pi_p; // inflation_obs   =  pi / pi_p;
  fpi_q_obs = i / i(-1) * muzstar / muzstar_p; // investment_obs  = i / i(-1) * muzstar / muzstar_p;
  @# if cee == 0
  networth_obs =  n / n(-1) * muzstar / muzstar_p; //networth_obs    = n / n(-1) * muzstar / muzstar_p;
  cp_q_obs     = exp((((G + omegabar * (1 - F)) - ((1 - mu_p) * G + omegabar * (1 - F))) * (1 + Rk) * q(-1) * kbar(-1)/ (q(-1) * kbar(-1) - n(-1))) - mu_p * G_ss * (1 + Rk_ss) * kbar_ss / (kbar_ss - n_ss)); 
  ...
  @# endif
  pinv_q_obs = muup(-1)/muup;    //pinvest_obs     = muup(-1) / muup;
  rff_q_obs =  exp(Re - Re_p); //Re_obs          = exp(Re - Re_p);
  RealRe_obs      = ((1 + Re) / pi(+1))/((1 + Re_p) / pi_p);
  @# if cee == 0
  spreadl_obs  = 1 + RL - Re; //Spread1_obs     = 1 + RL - Re;  
  @# endif
