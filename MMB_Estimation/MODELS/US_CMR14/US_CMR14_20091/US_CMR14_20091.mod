...
@# define financial_data = ["networth_q_obs", "credit_q_obs", "cp_q_obs", "spreadl_obs"]
...
@#define net_worth_in_financial_data = ("networth_q_obs" in financial_data)
...
@#define Spread1_in_financial_data = ("spreadl_obs" in financial_data)
...
@#define credit_in_financial_data = ("credit_q_obs" in financial_data)
@#define premium_in_financial_data = ("cp_q_obs" in financial_data)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %                   * (aptil_p * pi_p * log(pi(+1) / pitarget) + (1 / 4) * adytil_p * muzstar_p * (xgdp_q_obs));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  exp(pcer_q_obs) = c / c(-1) * muzstar / muzstar_p; // log(c) -log(c(-1)) + log(muzstar) -log(muzstar_p); //  consumption_obs = c / c(-1) * muzstar / muzstar_p;
  @#if cee == 0
  exp(credit_q_obs) = (q * kbar - n) / (q(-1) * kbar(-1) - n(-1)) * muzstar / muzstar_p; // credit_obs      = (q * kbar - n) / (q(-1) * kbar(-1) - n(-1)) * muzstar / muzstar_p;
  @#endif
  exp(xgdp_q_obs)  = (c + i / muup + g) / (c(-1) + i(-1) / muup(-1) + g(-1)) * muzstar / muzstar_p; //  gdp_obs         = (c + i / muup + g) / (c(-1) + i(-1) / muup(-1) + g(-1)) * muzstar / muzstar_p;
  exp(hours_obs) = h / h_ss; // hours_obs       = h / h_ss;
  exp(pgdp_q_obs)  = pi/pi_p; // inflation_obs   =  pi / pi_p;
  exp(fpi_q_obs) = i / i(-1) * muzstar / muzstar_p; // investment_obs  = i / i(-1) * muzstar / muzstar_p;
  @# if cee == 0
  exp(networth_q_obs) =  n / n(-1) * muzstar / muzstar_p; //networth_obs    = n / n(-1) * muzstar / muzstar_p;
  exp(cp_q_obs)     = exp((((G + omegabar * (1 - F)) - ((1 - mu_p) * G + omegabar * (1 - F))) * (1 + Rk) * q(-1) * kbar(-1)/ (q(-1) * kbar(-1) - n(-1))) - mu_p * G_ss * (1 + Rk_ss) * kbar_ss / (kbar_ss - n_ss)); 
  ...
  @# endif
  exp(pinv_q_obs) = muup(-1)/muup;    //pinvest_obs     = muup(-1) / muup;
  exp(rff_q_obs) =  exp(Re - Re_p); //Re_obs          = exp(Re - Re_p);
  RealRe_obs      = ((1 + Re) / pi(+1))/((1 + Re_p) / pi_p);
  @# if cee == 0
  exp(spreadl_obs)  = 1 + RL - Re; //Spread1_obs     = 1 + RL - Re;  
  @# endif
  exp(wage_obs)        = wtilde / wtilde(-1) * muzstar / muzstar_p;
...
stderr networth_q_obs, INV_GAMMA_PDF,  0.01,              5;
@# endif
@# endif
end;
...
stderr e_lambdaf,       0.00233333333333;// 0.0109559606811660;  
stderr e_muup,          0.00233333333333;// 0.0039857917638490;
stderr e_g,             0.00233333333333;// 0.0228127233194570;
stderr e_muzstar,       0.00233333333333;// 0.0071487847687080;
@# if cee == 0
stderr e_gamma,         0.00233333333333;// 0.0081035403472920;
@# endif
stderr e_epsil,         0.00233333333333;// 0.0046338119312200;
stderr e_xp,           	0.58333333333333;//  0.4893446208831330;
stderr e_zetac,         0.00233333333333;// 0.0233253551183060;
stderr e_zetai,         0.00233333333333;// 0.0549648244379450;
@# if cee == 0
@# if Spread1_in_financial_data
stderr e_term,          0.00233333333333;// 0.0016037530257270;
@# endif
@# if net_worth_in_financial_data
stderr networth_q_obs,   0.01;//  0.0174589738467390;
@# endif
@# endif
xiw_p,                   0.75; 		//0.8127963113950160;
b_p,                     0.5;		//0.7358438226929010;
@# if cee == 0
Fomegabar_p,             0.0075;	//0.0055885692972290;
mu_p,                    0.275;		//0.2148945111281970;
@# endif
sigmaa_p,                1; 		//2.5355534195260200;
Sdoupr_p,                5;			//10.7800000034422000;
xip_p,                   0.5;		//0.7412186033856290;
@# if taylor1p5 == 1
aptil_p,                 1.5;
@# else
aptil_p,                 1.5;		//2.3964959426752000;
@# endif
rhotil_p,                0.75;		//0.8502964502607260;
iota_p,                  0.5;		//0.8973670521349900;
iotaw_p,                 0.5;		//0.4890735358842230;
iotamu_p,                0.5;		//0.9365652807278990;
adytil_p,                0.25;		//0.3649436543356210; 

@# if cee == 0
@# if (some_financial_data && signal_corr_nonzero)
signal_corr_p,           0;			//0.3861343781103740;
@# endif
@# endif

rholambdaf_p,            0.5;		//0.9108528528580380;
rhomuup_p,               0.5;		//0.9870257396836700;
rhog_p,                  0.5;		//0.9427215849959780;
rhomuzstar_p,          	 0.5;		//0.1459051086113400;
rhoepsil_p,              0.5;		//0.8089285617540170;
@# if cee == 0
rhosigma_p,              0.5;		//0.9706370265612010;
@# endif 
rhozetac_p,              0.5;		//0.8968400853887450;
rhozetai_p,              0.5;		//0.9086616567125290;
@# if cee == 0
@# if Spread1_in_financial_data
rhoterm_p,               0.5;		//0.9743991813961140;
@# endif
@# if stopsignal == 0
stdsigma2_p,             0.000824957911384; //0.0282985295279650;
@# endif
...
@# endfor
@# endif
       pgdp_q_obs, hours_obs,  xgdp_q_obs,
       wage_obs, fpi_q_obs, pcer_q_obs,  
       rff_q_obs, pinv_q_obs;
